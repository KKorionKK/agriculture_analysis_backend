        ...
        if this_iter:
            cr_items.append(
                CrOrganizationsUsers(
                    organization_id=org.id, user_id=user_id, role=random.choice(roles)
                )
            )
    ...
    for i in range(count):
        users.append(
            User(
                ...
                password=AuthorizationService.hash_password(
                    "".join([str(random.randint(0, 100)) for i in range(10)])
                ),
                type="user",
            )
        )
...
def insert_field():
    ...
    organization = Organization(
        name="ОАО Огурцы и помидоры", is_public=True, owner_id=user.id
    )
